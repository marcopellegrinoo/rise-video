# -*- coding: utf-8 -*-
"""shap_st_cineca.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1YMFqukA3MZJeKU2Lf_jIhMxxVW6QvT1E
...
def shap_st_explain(nr_instance, data_test_image, data_test_OHE, models, channel, N):
  ...
  # Pesi scon formula Kernel-SHAP
  M = len(spatial_superpixels) * len(temporal_superpixels)
  weights = []
  ...
  regressor_for_positive = Ridge(alpha=0.0)
  regressor_for_positive.fit(X_for_positive, y_for_positive, sample_weight=weights)  # Pesi usati qui

  # Coefficienti calcolati dal modello
  coefficients_for_positive = regressor_for_positive.coef_

  regressor_for_negative = Ridge(alpha=0.0)
  regressor_for_negative.fit(X_for_negative, y_for_negative, sample_weight=weights)  # Pesi usati qui
...
for nr_instance, _ in enumerate(vottignasco_test_image):
  print(f"############### LIME-Spatio-Temporal with N:{N} on Instance #{nr_instance} ###############")
  ...
  saliency_video_i, total_coefficients_i = shap_st_explain(nr_instance, vottignasco_test_image, vottignasco_test_OHE, models, channel_prec, N)
  ...
  print("############### END INSTANCE Instance #{nr_instance} ###############################################################################")
...
df_stats_mean_insertion.to_csv(os.path.join(work_path, f"Water_Resources/rise-video/XAI/spatial_temporal/results/additive_uniform_noise/shap_all_stats_insertion.csv"), index=False)
df_stats_mean_deletion.to_csv(os.path.join(work_path, f"Water_Resources/rise-video/XAI/spatial_temporal/results/additive_uniform_noise/shap_all_stats_deletion.csv"),   index=False)
