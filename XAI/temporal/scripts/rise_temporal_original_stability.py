...
l = 16 #"???"   # Fixed values for l (BEST CONFIGURATION)
p = 0.5 #"???"   # Fixed value for p (BEST CONFIGURATION)

M = 10 #"???" # Number of random seeds to generate
seed_values = np.random.choice(100, M, replace=False)

len_test_set = len(vottignasco_test_image)

for seed in seed_values:
    param_combination = f"seed_{seed}"
    ...
    path_to_save_results = f"{RESULT_DIR}/rise_temporal_original_stability_{param_combination}.pkl"
